...
class Coach:
            ...
            history.append(
                {
                    ...
                    "last_note": last.notes if last else None,
                    "next_review": rec.next_review,
                    "due_for_review": review.is_due(rec, self.today),
                    "days_overdue": (
                        (self.today - date.fromisoformat(rec.next_review)).days
                        if rec.next_review
                        else None
                    ),
    ...
    def record_feedback(
        ...
        next_seq = 1 + max(
            (a.seq for rec in state.records.values() for a in rec.attempts),
            default=0,
        )
    ...
    # ---- roadmap browser (topic -> problem list, with star/status) ------ #

    def roadmap_overview(self) -> dict[str, Any]:
        state, roadmap = self._require_state()
            ...
            items = [
                {
                    ...
                    "difficulty": p.difficulty,
                    "order": p.order,
                    "solved": bool((rec := state.records.get(p.title)) and rec.solved),
                    "starred": p.title in starred,
                }
                for p in probs
            ]
            topics.append(
                {
                    "topic": topic,
                    "problems": items,
                    "solved": sum(1 for i in items if i["solved"]),
                    "total": len(items),
                }
            )

        return {"roadmap": roadmap.name, "topics": topics}

    def set_starred(self, title: str, starred: bool) -> None:
        state, roadmap = self._require_state()
        if roadmap.by_title(title) is None:
            raise ValueError(f"Unknown problem {title!r}")
        current = set(state.starred)
        if starred:
            current.add(title)
        else:
            current.discard(title)
        state.starred = sorted(current)
        self.store.save(state)
